...
public:
	...
	static bool GetVerticesFromSvgFile(const std::string& filePath, std::vector<std::vector<FVector2>>& vertices);
...
private:
	...
	static bool GetVerticesFromSvgString(std::string& svgText, std::vector<std::vector<FVector2>>& vertices);
	static bool GetVerticesFromPathData(const std::string& pathData, std::vector<FVector2>& vertices);
	...
	static FVector2 ReadSvgPoint(std::stringstream& stream);
	...
	static FVector2 FirstSvgPoint(std::stringstream& stream, FVector2& cursor, char cmd, bool isOpen, bool advance);

	// Read the next point, 
	// taking into account relative and absolute positioning.
	// Advances the cursor if requested.
	// Throws an exception if the figure is not open
	static FVector2 NextSvgPoint(std::stringstream& stream, FVector2& cursor, char cmd, bool isOpen, bool advance);

	// Reads next point, given only the new x coordinate
	static FVector2 NextSvgCoordX(std::stringstream& stream, FVector2& cursor, char cmd, bool isOpen);

	// Reads next point, given only the new y coordinate 
	static FVector2 NextSvgCoordY(std::stringstream& ssRef, FVector2& cursor, char cmd, bool isOpen);
};
