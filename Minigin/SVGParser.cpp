...
bool SVGParser::GetVerticesFromSvgFile(const std::string& filePath, std::vector<std::vector<FVector2>>& vertices)
{
	// Open the file
...
bool SVGParser::GetVerticesFromSvgString(std::string& svgString, std::vector<std::vector<FVector2>>& vertices)
{
	...
	while (GetElementContent(svgString, "path", pathElementContent, startPosContent, endPosContent))
	{
		...
		std::vector<FVector2> verticesVector;
...
bool SVGParser::GetVerticesFromPathData(const std::string& pathData, std::vector<FVector2>& vertices)
{
	...
	char cmd = 0;
	FVector2 cursor{};
	FVector2 startPoint;//At the end of the z command, the new current point is set to the initial point of the current subpath.
...
FVector2 SVGParser::ReadSvgPoint(std::stringstream& svgStream)
{
...
FVector2 SVGParser::FirstSvgPoint(std::stringstream& svgStream, FVector2& cursor, char cmd, bool isOpen, bool advance)
{
	if (!isOpen)
	{
	...
	FVector2 p = ReadSvgPoint(svgStream);
...
FVector2 SVGParser::NextSvgPoint(std::stringstream& svgStream, FVector2& cursor, char cmd, bool isOpen, bool advance)
{
	if (isOpen)
	{
...
FVector2 SVGParser::NextSvgCoordX(std::stringstream& svgStream, FVector2& cursor, char cmd, bool isOpen)
{
	if (isOpen)
	{
...
FVector2 SVGParser::NextSvgCoordY(std::stringstream& svgStream, FVector2& cursor, char cmd, bool isOpen)
{
	if (isOpen)
	{
